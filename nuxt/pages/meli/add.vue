<template>
  <div class="container">
    ...
    <h2 class="subtitle">Aplicá las configuraciones de publicaciones que te gustaría que tus productos cumplan</h2>
    <hr />
    <div class="columns">
      <div class="column">
      ...
      <div class="column">
        Agregar porcentaje:
        <br />
        <input type="text" class="input" v-model="percent" placeholder="1.13%" />
      </div>
      ...
      <div class="column">
        <br />
        <button @click="addEach" class="button is-primary">SUBIR A MERCADOLIBRE</button>
      </div>
    </div>
    <hr />
    <div class="notification is-info text-uppercase bold fs-13">
      Resultado de productos publicados exitosamente <span class="fs-16">({{ total }})</span>
    </div>
    <div class="row">
      <div class="col-md-12">
...
export default {
  ...
  methods: {
    addEach: async function() {
      var checkToken = await axios.get(process.env.apiUrl + "/mercadolibre/refresh-token");
